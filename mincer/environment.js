...
var environment = module.exports = new mincer.Environment(root);
...
environment.appendPath(path.join(root, 'client'));
...
_.each([ 'fonts', 'images', 'javascripts', 'stylesheets' ], function(type) {
  environment.appendPath(path.join(root, 'vendor', 'assets', type));
});
...
environment.cache = new mincer.FileStore(path.join(root, 'cache'));
